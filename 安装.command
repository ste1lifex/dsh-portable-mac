...
#   未签名的 App 在 macOS 上首次启动会被 Gatekeeper 拦截（提示「无法验证开发者」
#   或「已损坏」），而且 App 内部被修改过、原签名已失效。
#   本脚本做三件事即可正常使用：
#     1) 解除下载隔离标记（quarantine）
#     2) 修复可执行权限
#     3) 用 ad-hoc（-）临时签名重新签名，让系统接受
#
...
echo "[3/4] 临时签名（ad-hoc，无需开发者账号）…"
echo "      包体较大，预计需要 1-3 分钟，请勿关闭窗口…"
